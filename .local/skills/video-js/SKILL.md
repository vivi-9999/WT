...
*Illustration only — small millisecond values so the arithmetic is easy to read. Production `SCENE_DURATIONS` are typically larger (see `<video_structure>` for the recipe); the same padding rules apply at any scale.*
...
- Always vary scene durations -- if every scene is the same length the rhythm dies. Most videos land 30-60s total (per-scene ~3-10 s); the **total** must stay **≤ 120_000** ms; mix short punchy beats with longer dramatic holds, sized to the content rather than padded to the cap.
</visual_style>
...
```tsx
// Per-scene hold times in ms. Cap total ≤ 120_000 ms (~2 minutes); most videos land 30-60s.
// Size the sum to the content, not to the cap.
...
- **Aim for at least 5 scenes** (more is fine). **The sum of all `SCENE_DURATIONS` must be ≤ ~120_000 ms** (the ~2 minute export ceiling); **most videos land 30-60s** — only push longer when the brief truly needs it. Name the keys to fit your content; the example uses generic keys but you should use descriptive names for the specific video.
...
- **Size to content. 30-60s is the typical range; ≤ ~120_000 ms (2 minutes) is the hard ceiling.** With 5+ scenes, that's often **~3-10 seconds per scene** (3_000-10_000 ms) on average; scenes can run longer for dramatic holds when the content supports it. Don't pad scene durations just to reach the cap — longer videos are harder to export and usually feel slow.
...
export function Scene1() {
  ...
  return (
    <motion.div className="absolute inset-0 flex items-center justify-center"
      ...
      {/* Midground: floating accent shape — duration ≤ scene duration (open: 5000 → 5s) */}
...
4. Define `SCENE_DURATIONS` sized to the content — most videos land 30-60s total with per-scene ~3-10 s; the **sum must stay ≤ 120_000 ms** (the export ceiling). Don't pad to the cap; vary pacing within the content's arc.
...
6. **Build at least 5 scenes** (more if the arc needs it), each in its own file under `src/components/video/video_scenes/` -- e.g. `Scene1.tsx` through `Scene5.tsx`. Each scene is a choreographed sequence, not a single entrance animation. Plan multiple timed moments per scene using delays or `useEffect` with `setTimeout`. Each scene should have background, midground, and foreground layers. Scene **durations** should sum to the content's natural length (typically 30-60s; **≤ 120_000 ms** always).
...
2. **Verify frame containment, loop integrity, and motion smoothness yourself** (do not delegate -- subagents cannot spawn nested subagents). Walk through each scene file and check:
   ...
   - No `transition.duration` (Framer Motion seconds) inside a scene exceeds that scene's `SCENE_DURATIONS` value divided by 1000 (`SCENE_DURATIONS` is in ms; `transition.duration` is in s — a `duration: 4` inside a `5_000` ms scene is fine; a `duration: 8` inside a `3_500` ms scene would be wrong)
