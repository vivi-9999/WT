...
async function downloadResults(report) {
    ...
    const addHeader = (pageNum) => {
        ...
        doc.text("WVSS — Security Scan Report", M, 14);
    ...
    doc.text("Generated by WVSS — For authorized security research and educational use only.", W/2, H - 3.5, {align:"center"});

    doc.save(`wvss-results-${target.replace(/[^a-z0-9]/gi,"_").slice(0,30)}-${Date.now()}.pdf`);
}
...
function downloadLogs(report) {
    ...
    a.download = `wvss-log-${target.replace(/[^a-z0-9]/gi,"_").slice(0,30)}-${Date.now()}.txt`;
    a.click();
}
